...
func (e *Engine) Client_coin() ([]map[string][]byte, error) {
	...
	resulist, _ := e.Engine.Query("SELECT\n\td1.NAME AS 部门,\n\td.NAME AS 二级部门,\n\tu.nickname 员工姓名,\n\tu.login_name AS 员工工号,\n\tcc.customer_code AS 客户编码,\n\tcc.NAME AS 客户姓名,\n\tcc.coin AS 客户积分 \nFROM\n\tbl_crm_customer cc\n\tINNER JOIN bl_crm_customer_phone ccp ON cc.id = ccp.customer_id\n\tINNER JOIN bl_crm_customer_append cca ON cc.id = cca.customer_id\n\tINNER JOIN bl_users u ON cc.user_id = u.id\n\tINNER JOIN bl_depart d ON u.depart_id = d.id\n\tINNER JOIN bl_depart d1 ON d.parent_id = d1.id \nWHERE\n\tcca.first_category = 1 \n\tAND cc.deleted_at IS NULL \n\tAND cc.coin > 0;")
