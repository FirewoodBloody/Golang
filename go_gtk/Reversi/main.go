...
//函数，给按钮添加图片
func ButtonFromImageFile(button *gtk.Button, filename string) {
	//获取按钮宽度和高度
	var w, h int
	...
	button.SetImage(gtk.NewImageFromPixbuf(pixbuf)) //获取image 并设置按钮图片
...
func (obj *Chessboard) CreateWindow() *gtk.Window {
	...
	builder.AddFromFile("window.glade") //读取glade窗口文件

	//获取窗口控件,按钮控件,label 控件按钮,image控件
	obj.ChessWidet.window = gtk.WindowFromObject(builder.GetObject("window1"))
	obj.buttonMin = gtk.ButtonFromObject(builder.GetObject("button1"))
	obj.buttonClose = gtk.ButtonFromObject(builder.GetObject("button2"))
	obj.labelBlack = gtk.LabelFromObject(builder.GetObject("label1"))
	obj.labelWhite = gtk.LabelFromObject(builder.GetObject("label2"))
	obj.labelTime = gtk.LabelFromObject(builder.GetObject("label3"))
	obj.imageBlack = gtk.ImageFromObject(builder.GetObject("image1"))
	obj.imageWhite = gtk.ImageFromObject(builder.GetObject("image2"))
	...
	//设置窗口属性
	obj.w, obj.h = 800, 500
	obj.window.SetDefaultSize(obj.w, obj.h)    //设置窗口大小
	obj.window.SetPosition(gtk.WIN_POS_CENTER) //设置窗口居中
	obj.window.SetAppPaintable(true)           //允许窗口绘图
	obj.window.SetDecorated(false)             //去除表框

	//设置按钮属性
	ButtonFromImageFile(obj.buttonMin, "./image/min.png") //给按钮设置图片，此为自定义函数
	ButtonFromImageFile(obj.buttonClose, "./image/close.png")
	...
	obj.labelWhite.ModifyFG(gtk.STATE_NORMAL, gdk.NewColor("green"))
	obj.labelBlack.ModifyFG(gtk.STATE_NORMAL, gdk.NewColor("green"))
...
func main() {
	...
	window := obj.CreateWindow() //创建控件，设计属性
