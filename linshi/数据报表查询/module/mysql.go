...
func Order_select(w *Windows, f *excelize.File) {
	...
	for k, v := range resulist {
		...
		f.SetCellValue("Sheet1", fmt.Sprintf("Y%v", k+2), v["商品总价"])
			...
			if string(v["快递单号"]) != "" {
				if string(v["配送方式"]) == "顺丰快递" {
					...
					if len(data.Body.RouteResponse.Route) != 0 {
						...
						f.SetCellValue("Sheet1", fmt.Sprintf("AB%v", k+2), data.Body.RouteResponse.Route[len(data.Body.RouteResponse.Route)-1].Remark)
					}
				} else if string(v["配送方式"]) == "京东快递" {
					data := modules.SelectData(string(v["快递单号"]))
					if len(data.Data) != 0 {
						//fmt.Println(data.Data[len(data.Data)-1].OpeRemark)
						f.SetCellValue("Sheet1", fmt.Sprintf("AB%v", k+2), data.Data[len(data.Data)-1].OpeRemark)
					}
				}
			}
		} else {
...
func Order_xs_select(w *Windows, f *excelize.File) {
	...
	for k, v := range resulist {
		if k == 0 {
			...
			f.SetCellValue("Sheet1", fmt.Sprintf("X%v", k+1), "客户地址")
		}
		...
		f.SetCellValue("Sheet1", fmt.Sprintf("L%v", k+2), v["系统订单号"])
		f.SetCellValue("Sheet1", fmt.Sprintf("M%v", k+2), v["订单类型"])
		f.SetCellValue("Sheet1", fmt.Sprintf("N%v", k+2), v["订单状态"])
		f.SetCellValue("Sheet1", fmt.Sprintf("O%v", k+2), v["订单总金额"])
		f.SetCellValue("Sheet1", fmt.Sprintf("P%v", k+2), v["订单商品"])
		f.SetCellValue("Sheet1", fmt.Sprintf("Q%v", k+2), v["商品数量"])
		f.SetCellValue("Sheet1", fmt.Sprintf("R%v", k+2), v["商品原价"])
		f.SetCellValue("Sheet1", fmt.Sprintf("S%v", k+2), v["商品售价"])
		f.SetCellValue("Sheet1", fmt.Sprintf("T%v", k+2), v["商品总价"])
		f.SetCellValue("Sheet1", fmt.Sprintf("U%v", k+2), v["配送方式"])
		f.SetCellValue("Sheet1", fmt.Sprintf("V%v", k+2), v["快递单号"])
		f.SetCellValue("Sheet1", fmt.Sprintf("X%v", k+2), v["客户地址"])
		...
		if string(v["订单状态"]) == "订单配送中" || string(v["订单状态"]) == "订单未妥投" {
			if string(v["快递单号"]) != "" {
				if string(v["配送方式"]) == "顺丰快递" {
					data, _ := express.SfCreateData(string(v["快递单号"]))
					if len(data.Body.RouteResponse.Route) != 0 {
						f.SetCellValue("Sheet1", fmt.Sprintf("W%v", k+2), data.Body.RouteResponse.Route[len(data.Body.RouteResponse.Route)-1].Remark)
					}
				} else if string(v["配送方式"]) == "京东快递" {
					data := modules.SelectData(string(v["快递单号"]))
					if len(data.Data) != 0 {
						f.SetCellValue("Sheet1", fmt.Sprintf("W%v", k+2), data.Data[len(data.Data)-1].OpeRemark)
					}
				}
			}
		} else {
			f.SetCellValue("Sheet1", fmt.Sprintf("W%v", k+2), v["快递状态"])
		}
		vcl.ThreadSync(func() {
			w.Progress_bar.SetPosition(int32(float64(k+1) / float64(len(resulist)) * 100))
		})
...
func Client_coin(w *Windows, f *excelize.File) {
	...
	for k, v := range resulist {
		if k == 0 {
			...
			f.SetCellValue("Sheet1", fmt.Sprintf("C%v", k+1), "员工姓名")
			f.SetCellValue("Sheet1", fmt.Sprintf("D%v", k+1), "员工工号")
			f.SetCellValue("Sheet1", fmt.Sprintf("E%v", k+1), "客户编码")
			f.SetCellValue("Sheet1", fmt.Sprintf("F%v", k+1), "客户姓名")
			f.SetCellValue("Sheet1", fmt.Sprintf("G%v", k+1), "客户积分")
		...
		f.SetCellValue("Sheet1", fmt.Sprintf("C%v", k+2), v["员工姓名"])
		f.SetCellValue("Sheet1", fmt.Sprintf("D%v", k+2), v["员工工号"])
		f.SetCellValue("Sheet1", fmt.Sprintf("E%v", k+2), v["客户编码"])
		f.SetCellValue("Sheet1", fmt.Sprintf("F%v", k+2), v["客户姓名"])
		f.SetCellValue("Sheet1", fmt.Sprintf("G%v", k+2), v["客户积分"])
...
func Client_birthday(w *Windows, f *excelize.File) {
	...
	for k, v := range resulist {
		if k == 0 {
			...
			f.SetCellValue("Sheet1", fmt.Sprintf("E%v", k+1), "员工姓名")
			f.SetCellValue("Sheet1", fmt.Sprintf("F%v", k+1), "员工工号")
			f.SetCellValue("Sheet1", fmt.Sprintf("G%v", k+1), "消费合计")
			f.SetCellValue("Sheet1", fmt.Sprintf("H%v", k+1), "最大单笔消费")
		...
		f.SetCellValue("Sheet1", fmt.Sprintf("E%v", k+2), v["员工姓名"])
		f.SetCellValue("Sheet1", fmt.Sprintf("F%v", k+2), v["员工工号"])
		f.SetCellValue("Sheet1", fmt.Sprintf("G%v", k+2), v["消费合计"])
		f.SetCellValue("Sheet1", fmt.Sprintf("H%v", k+2), v["最大单笔消费"])
...
func Client_prestigious_university(w *Windows, f *excelize.File) {
	...
	for k, v := range resulist {
		if k == 0 {
			...
			f.SetCellValue("Sheet1", fmt.Sprintf("D%v", k+1), "员工工号")
			f.SetCellValue("Sheet1", fmt.Sprintf("E%v", k+1), "已购客户")
			f.SetCellValue("Sheet1", fmt.Sprintf("F%v", k+1), "准已购客户")
			f.SetCellValue("Sheet1", fmt.Sprintf("G%v", k+1), "意向客户")
			f.SetCellValue("Sheet1", fmt.Sprintf("H%v", k+1), "潜在客户")
		...
		f.SetCellValue("Sheet1", fmt.Sprintf("D%v", k+2), v["员工工号"])
		f.SetCellValue("Sheet1", fmt.Sprintf("E%v", k+2), v["已购客户"])
		f.SetCellValue("Sheet1", fmt.Sprintf("F%v", k+2), v["准已购客户"])
		f.SetCellValue("Sheet1", fmt.Sprintf("G%v", k+2), v["意向客户"])
		f.SetCellValue("Sheet1", fmt.Sprintf("H%v", k+2), v["潜在客户"])
